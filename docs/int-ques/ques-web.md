...
## 描述下 Cookies、SessionStorage和LocalStorage区别
...
> iframe：是HTML标签，作用是文档中的文档，或者浮动的框架(FRAME)
>
> onload ：加载
...
> 更多请参照网上 [meta标签大全](https://blog.csdn.net/qq_16559905/article/details/51182085)


## ⻚⾯在哪些流览器测试过？这些浏览器的内核分别是什么
...
- src 是 source 的缩写（来源），指向外部资源的位置，指向的内容将会嵌⼊到⽂档中当前标签所 在位置；在请求 src 资源时会将其指向的资源下载并应用到⽂档内，例如 js 脚本，
...
> 当浏览器解析到该元素时，会暂停其他 资源的下载和处理，直到将该资源加载、编译、执⾏完毕，图⽚和框架等元素也如此，类似于将所指向资源嵌⼊当前标签内。这也是为什么将js脚本放在底 部⽽不是头部
...
> 那么浏览器会识别该⽂档为 css ⽂件，就会进行下载资源并且不会停⽌对当前⽂档的处理。这也是为什么建议使⽤ link 方式来加载 css ，而不是使⽤ @import ⽅式 
>
> 因为 @import ⽅式加载样式，是等页面记载完成之后再加载样式
...
jpg和JPEG两种图片格式有什么区别？详见：[jpg和JPEG的区别](https://zhidao.baidu.com/question/1497075542190008939.html)
...
- Webp： WebP 格式，谷歌（google）开发的⼀种旨在加快图⽚加载速度的图⽚格式。图 

  片压缩体积⼤约只有 JPEG 的 2/3 ，并能节省⼤量的服务器带宽资源和数据空间
...
- Apng：全称是 Animated Portable Network Graphics（便携式网络动画） , 是PNG的位图动画扩展，可 以实现png格式的动态图⽚效果。04年诞⽣，但⼀直得不到各⼤浏览器⼚商的⽀持，直到目前得到 iOS safari 8 的⽀持，有望代替 GIF 成为下⼀代动态图标准

> iOS safari 8：苹果系统8浏览器
...
## 一个页面上有大量的图片，加载很慢，你有那些方法优化这些图片的加载，给用户更好的体验
...
npm 是世界上最大的软件注册表，拥有一百多万个软件包， Node.js 默认的、以JavaScript 编写的软件包管理系统。如果一个程序依赖了很多包的话，那么没有一个包管理系统来维护的话，那么需要自己来解决这些依赖关系，就会非常的麻烦，那么有了 npm，就可以一条命令下来，它就可以帮你把依赖关系和对应的版本帮你管理了，这就是 npm 最大的一个好处。
...
- link 最⼤限度⽀持并⾏下载， @import 过多嵌套导致串⾏下载，出现 FOUC (⽂档样式 

  短暂失效) 

- link 可以通过 rel="alternate stylesheet" 指定候选样式

- link 可以通过 js 来动态引入样式

- 浏览器对 link ⽀持早于 @import ，可以使⽤ @import 对⽼浏览器隐藏样式

- @import 必须在样式规则之前，可以在css⽂件中引⽤其他⽂件

- 加载方法不一样。 link 引入是先加载样式再加载页面，但是 @import 是页面加载完成后加载样式
...
>
> BFC 是一个独立的区域，让 BFC 内部的元素与外部的元素相互隔离
>
> 也就是内外元素互不影响
...
也就是字面意思即，用更简练的言语表达较复杂的含义。在得到广泛接受的情况之下，可以提升交流的效率。
