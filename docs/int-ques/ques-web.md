...
- 对于⼀些⼩图标，可以使⽤ base64 位编码，以减少⽹络请求。但不建议⼤图使⽤，⽐较耗
...
- 前端模板 JS+数据，减少由于 HTML 标签导致的带宽浪费，前端⽤变量保存 AJAX 请求结
...
## 在 css/js 代码上线之后开发人员经常会优化性能，从用户刷新网页开始，⼀次 js 请求⼀般情况下有哪些地方会有缓存处理
...
- 如果图⽚为 css 图⽚，可以使⽤ CSSsprite ， SVGsprite ， Iconfont 、 Base64 等技
...
npm 是世界上最大的软件注册表，拥有一百多万个软件包， Node.js 默认的、以 JavaScript 编写的软件包管理系统。如果一个程序依赖了很多包的话，那么没有一个包管理系统来维护的话，那么需要自己来解决这些依赖关系，就会非常的麻烦，那么有了 npm，就可以一条命令下来，它就可以帮你把依赖关系和对应的版本帮你管理了，这就是 npm 最大的一个好处。
